...
import styles from "./PlayEasy.module.scss";
...
const PlayEasy = () => {
  ...
  useEffect(() => {
    ...
    socket.on("coupjoued", (data) => {
      ...
      socket.emit("game_mode", { mode: "computerplayeasy" });
    });

    socket.on("move_from_ai", (data) => {
      playMove(data.move_from_ai);
    });

    return () => {
      socket.off("connect");
      socket.off("coupjoued");
      socket.off("move_from_ai");
      socket.close();
    };
  ...
  function onDrop(sourceSquare, targetSquare) {
    try {
      ...
      toast.error("Erreur lors de la tentative de coup: " + error.message, {
        position: "top-center",
        autoClose: 3000,
        hideProgressBar: true,
        closeOnClick: true,
      ...
      <div className={styles.chessBoardButton}>
        <button
          ...
          onClick={() => {
            ...
            socket.emit("undo", { mode: game.fen() });
          }}
        >
          Undo
        </button>
